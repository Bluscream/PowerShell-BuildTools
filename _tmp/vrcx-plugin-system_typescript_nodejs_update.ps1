...
# Builds, tests, and optionally publishes the plugin system
# Usage: .\update.ps1 [-Publish]
...
if ($hasGit) {
    ...
    if ($Publish -and $hasGh -and $BuildResults.Core.Committed) {
...
"@
    ...
    elseif ($Publish) {
        Write-Warning "GitHub release skipped (use -Publish flag to create release)"
    }
}
else {
